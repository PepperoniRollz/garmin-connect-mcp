...
| `create-workout` | **Write (Garmin).** Creates a structured strength workout in Garmin Connect from `name` + `exercises[]` of `{exercise, sets, reps, targetWeight?, restSeconds?}`; optional `scheduleDate` puts it on the calendar | Syncs to the watch under Training > Workouts. `exercise` is one of eight configured lifts (`src/constants.ts` `LIFT_EXERCISES`); `targetWeight` is in your Garmin display unit (lb/kg auto-detected from settings); `restSeconds` makes timed rests, omitted = lap-button rest |
