...
| `get-strength-sets` | Per-set strength breakdown of a lifting session for post-workout **review**: each exercise with its sets (`reps`, `weightLb`, `restSec`), conditioning context (duration, avg/max HR, total rest, calories), and a human-readable summary. Pass `activityId`, `date` (that day's strength session), or neither (newest) | Reps/weight are validated on the watch during each set, so this is the primary lifting record. Weight is integer **grams** → **lb** (raw `weightGrams` retained). Auto-detected exercises below 90% confidence (or `UNKNOWN`) are surfaced in `lowConfidenceFlags` to verify. Read-only — pairs with `confirm-strength-session` to commit |
| `confirm-strength-session` | **Write.** Imports a reviewed strength session (`activityId`) into the lift log so it counts toward progression. One logged session per exercise (`source: garmin`), keyed by activity id | Idempotent — re-confirming the same activity never double-imports. Garmin supersedes a manual entry on the same date+lift (reported as `supersededManual`). Garmin keys map back to lift shorthands (`BARBELL_ROW` → `barbell-row`); unmapped keys import under a normalized name and are flagged. Review with `get-strength-sets` first |
...
| `log-lift` | **Write.** Logs a lifting session (`lift`, `sets[]` of `{weight, reps}`, optional `date`/`note`) and returns a double-progression assessment | The manual **fallback** for watchless sessions and corrections — Garmin (via `confirm-strength-session`) is the primary source. Stored in a separate lift DB (`LIFT_DB_PATH`), never in the auth DB. All sets at the rep target → suggests adding weight (lower-body lifts get the larger jump); otherwise hold and beat the logbook |
| `get-lift-history` | Logged sessions newest-first, each with top-set weight, total reps, and `source` (`manual`/`garmin`) (optional `lift` filter, `limit`) | Unified log: imported Garmin sessions alongside manual ones |
| `get-lift-progress` | Weight progression over time for one `lift` (each point tagged `source`), plus current working weight and whether you're due to add weight | Unified log: imported Garmin sessions are primary, manual entries fill watchless days |
