...
if (!ownerPassword) {
  console.error('Set OAUTH_TEST_PASSWORD to the owner password used for the test server.');
  process.exit(1);
}
...
const unauth = await fetch(mcpUrl, {
  ...
  body: JSON.stringify({jsonrpc: '2.0', id: 0, method: 'initialize', params: {protocolVersion: '2025-06-18', capabilities: {}, clientInfo: {name: 't', version: '0'}}}),
});
const wwwAuth = unauth.headers.get('www-authenticate') ?? '';
check('unauthenticated MCP request returns 401', unauth.status === 401, `status ${unauth.status}`);
check('401 carries WWW-Authenticate with resource_metadata', wwwAuth.includes('resource_metadata='), wwwAuth);

// --- 2. RFC 9728 protected resource metadata ---
const prmMatch = wwwAuth.match(/resource_metadata="([^"]+)"/);
const prmUrl = prmMatch?.[1] ?? new URL('/.well-known/oauth-protected-resource/mcp', baseUrl).href;
const prm = await (await fetch(prmUrl)).json() as {resource: string; authorization_servers: string[]};
check('PRM resource exactly equals public MCP URL', prm.resource === mcpUrl.href, prm.resource);
check('PRM lists an authorization server', prm.authorization_servers.length === 1, prm.authorization_servers[0]);

// --- 3. RFC 8414 authorization server metadata ---
const asMetaUrl = new URL('/.well-known/oauth-authorization-server', prm.authorization_servers[0]);
...
check('AS metadata served', typeof asMeta.authorization_endpoint === 'string', asMeta.authorization_endpoint);
...
const badReg = await fetch(asMeta.registration_endpoint, {
  ...
  body: JSON.stringify({redirect_uris: [DISALLOWED_CALLBACK], token_endpoint_auth_method: 'none',
    grant_types: ['authorization_code', 'refresh_token'], response_types: ['code']}),
});
check('DCR with disallowed redirect URI rejected', badReg.status === 400, `status ${badReg.status}`);

// --- 5. DCR: claude.ai + loopback redirect URIs accepted ---
const reg = await fetch(asMeta.registration_endpoint, {
  method: 'POST',
  headers: {'content-type': 'application/json'},
  body: JSON.stringify({client_name: 'phase3-acceptance', redirect_uris: [CLAUDE_CALLBACK, LOOPBACK_CALLBACK],
...
check('authorize renders login page with pending id', authorizePage.status === 200 && pendingMatch !== null);
...
const badLogin = await fetch(new URL(LOGIN_PATH, baseUrl), {
  ...
  body: new URLSearchParams({pending_id: pendingId, password: 'wrong-password-123'}),
  redirect: 'manual',
});
...
check('correct password redirects with code', goodLogin.status === 302 && code !== '');
...
const tokenResp = await fetch(asMeta.token_endpoint, {
  ...
  body: new URLSearchParams({grant_type: 'authorization_code', code, code_verifier: verifier,
    redirect_uri: LOOPBACK_CALLBACK, client_id: client.client_id}),
});
const tokens = await tokenResp.json() as {access_token: string; refresh_token: string; expires_in: number; token_type: string};
check('token exchange succeeds', tokenResp.status === 200 && tokens.access_token !== undefined,
...
check('authorization code replay rejected', replay.status === 400, `status ${replay.status}`);
...
check('authenticated MCP session lists tools', tools.length === 13, `${tools.length} tools`);
...
check('garbage bearer token rejected with 401', garbage.status === 401, `status ${garbage.status}`);
...
check('refresh grant issues new tokens', refreshResp.status === 200 && refreshed.access_token !== tokens.access_token);
const reusedRefresh = await fetch(asMeta.token_endpoint, {
  method: 'POST',
  headers: {'content-type': 'application/x-www-form-urlencoded'},
  body: new URLSearchParams({grant_type: 'refresh_token', refresh_token: tokens.refresh_token, client_id: client.client_id}),
});
check('rotated (old) refresh token rejected', reusedRefresh.status === 400, `status ${reusedRefresh.status}`);
