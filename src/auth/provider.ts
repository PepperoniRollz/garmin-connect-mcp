...
export class OwnerAuthorizationProvider implements OAuthServerProvider {
  ...
  get clientsStore(): OAuthRegisteredClientsStore {
    return {
      ...
      registerClient: (client: OAuthClientInformationFull) => {
        const disallowed = client.redirect_uris.filter((uri) => !isAllowedRedirectUri(uri));
        if (disallowed.length > 0) {
          logger.info('audit', {event: AuditEvent.ClientRegistrationRejected, redirectUris: disallowed});
        ...
        logger.info('audit', {event: AuditEvent.ClientRegistered, clientId: client.client_id});
        return client;
      },
    };
  }
  ...
  async authorize(client: OAuthClientInformationFull, params: AuthorizationParams, res: Response): Promise<void> {
    ...
    if (!isAllowedRedirectUri(params.redirectUri)) {
      res.status(400).type('html').send(renderErrorPage('The redirect URI is not on this server\'s allowlist.'));
      return;
    }
    ...
    res.status(200).type('html').send(renderLoginPage({pendingId, clientName: client.client_name}));
  }
  ...
  async exchangeAuthorizationCode(
    ...
    const tokens = this.issueTokens(client.client_id, record.scopes, record.resource);
    logger.info('audit', {event: AuditEvent.TokenIssued, clientId: client.client_id});
    return tokens;
  }
  ...
  async exchangeRefreshToken(
    ...
    if (scopes !== undefined && scopes.some((scope) => !grantedScopes.includes(scope))) {
    ...
    const tokens = this.issueTokens(client.client_id, scopes ?? grantedScopes, record.resource);
    logger.info('audit', {event: AuditEvent.TokenRefreshed, clientId: client.client_id});
    return tokens;
  }
  ...
  async verifyAccessToken(token: string): Promise<AuthInfo> {
    ...
    return {
      ...
      resource: record.resource !== undefined ? new URL(record.resource) : undefined,
    };
  }

  async revokeToken(client: OAuthClientInformationFull, request: OAuthTokenRevocationRequest): Promise<void> {
    const token = request.token;
    const record = this.db.getToken(token, TokenKind.Access) ?? this.db.getToken(token, TokenKind.Refresh);
    if (record === undefined || record.clientId !== client.client_id) {
      return; // Per RFC 7009, revoking an unknown token is a no-op.
    }
    this.db.deleteToken(token);
    logger.info('audit', {event: AuditEvent.TokenRevoked, clientId: client.client_id});
  }

  /** Used by the login route after the owner password checks out. */
  issueAuthorizationCode(pendingId: string): {code: string; redirectUri: string; state?: string} | undefined {
