...
export function createLoginRouter(provider: OwnerAuthorizationProvider, ownerPasswordHash: string): Router {
      ...
      async (req: Request, res: Response) => {
        ...
        if (issued === undefined) {
          ...
          res.status(400).type('html').send(
              renderErrorPage('This sign-in link has expired. Return to the client and connect again.'));
          return;
        }
